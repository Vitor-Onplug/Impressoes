<?php
class PermissoesController extends MainController
{
    ...
    public function index()
    {
        ...
        if(!$this->check_permissions('sistema', $this->userdata['modulo']))
        {
		...
		if(chk_array($this->parametros, 0) == 'adicionar'){
			...
			$conteudo = ABSPATH . '/views/permissoes/frmPermissao.inc.view.php';
		}elseif(chk_array($this->parametros, 0) == 'editar'){
			$this->title = SYS_NAME . ' - Editar Permissão';
			
			$conteudo = ABSPATH . '/views/permissoes/frmPermissao.inc.view.php';
		}else{
			$conteudo = ABSPATH . '/views/permissoes/permissoes.view.php';
		}
