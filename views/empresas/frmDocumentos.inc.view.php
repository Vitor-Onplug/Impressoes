...
if(chk_array($parametros, 3) == 'editar' || chk_array($parametros, 3) == 'remover' ){
	if(empty(chk_array($parametros, 4)) || !is_numeric(chk_array($parametros, 4))){
		echo '<meta http-equiv="refresh" content="0; url=' . HOME_URI . '/empresas/index/editar/' . chk_array($this->parametros, 1) . '/documentos">';
		echo '<script type="text/javascript">window.location.href = "' . HOME_URI . '/empresas/index/editar/' . chk_array($this->parametros, 1) . '/documentos";</script>';
											...
											<div class="row">
												<div class="col-md-6">
													...
													<select class="form-control" name="tipo" id="tipo" required>
														...
														<option <?php if(htmlentities(chk_array($modeloDocumentos->form_data, 'tipo')) == 'CNPJ'){ echo'selected'; } ?> value="CNPJ">CNPJ</option>
														<option <?php if(htmlentities(chk_array($modeloDocumentos->form_data, 'tipo')) == 'CPF'){ echo'selected'; } ?> value="CPF">CPF</option>
														<option <?php if(htmlentities(chk_array($modeloDocumentos->form_data, 'tipo')) == 'IE'){ echo'selected'; } ?> value="IE">IE</option>
												...
												<div class="col-md-12">
													<div class="form-group">
														...
														<input type="text" class="form-control " id="documento" name="documento" placeholder="documento" value="<?php echo htmlentities(chk_array($modeloDocumentos->form_data, 'documento')); ?>" required maxlength="255">
													</div>
												</div>
										...
										<table id="table" class="table table-hover table-bordered table-striped dataTable">
											...
											<tbody>
												...
												<tr>
													...
													<td>
														<a href="<?php echo HOME_URI; ?>/empresas/index/editar/<?php echo chk_array($parametros, 1); ?>/documentos/editar/<?php echo $dados['id']; ?>" class="icon-tab" title="Editar"><i class="far fa-edit fa-lg"></i></a>
														&nbsp;
														<a href="<?php echo HOME_URI;?>/empresas/index/editar/<?php echo chk_array($parametros, 1); ?>/documentos/remover/<?php echo $dados['id']; ?>" class="icon-tab" title="Remover"><i class="fas fa-times fa-lg text-red"></i></a>
													</td>
												</tr>
												<?php endforeach; ?>
											</tbody>
