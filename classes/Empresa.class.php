...
class Empresa {
    ...
    public function validarEmpresa($razaoSocial = null, $nomeFantasia = null, $observacoes = null){ 
        ...
        if(empty($this->razaoSocial)){ $this->erro .= "<br>Preencha a razão social."; }
        if(!empty($this->razaoSocial)){
            if(strlen($this->razaoSocial) > 255){ $this->erro .= "<br>A razão social não pode ultrapassar o limite de 255 caracteres."; }
        }
        if(empty($this->nomeFantasia)){ $this->erro .= "<br>Preencha o nome fantasia."; }
        if(!empty($this->nomeFantasia)){
            if(strlen($this->nomeFantasia) > 255){ $this->erro .= "<br>O nome fantasia não pode ultrapassar o limite de 255 caracteres."; }
        }
