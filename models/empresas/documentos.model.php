...
class DocumentosModel extends MainModel {
	...
	public function validarFormDocumento(){ 
		...
		if(chk_array($this->parametros, 3) == 'editar'){
			$validaDocumento = $this->ClasseDocumento->validarDocumento(0, $this->idEmpresa, $this->tipo, $this->titulo, $this->documento, true, $this->detalhes);
		}else{
			$validaDocumento = $this->ClasseDocumento->validarDocumento(0, $this->idEmpresa, $this->tipo, $this->titulo, $this->documento, false, $this->detalhes);
		}
		...
		if(chk_array($this->parametros, 3) == 'editar'){
			...
			$this->adicionarDocumentoEmpresa();
		}
	...
	private function editarDocumento(){
		...
		if(!$editaDocumento){
			...
			$this->form_msg .= '<meta http-equiv="refresh" content="2; url=' . HOME_URI . '/empresas/index/editar/' . chk_array($this->parametros, 1) .'/documentos/">';
		}
		
		return;
	}
	
	private function adicionarDocumentoEmpresa(){
		$insereDocumento = $this->ClasseDocumento->adicionarDocumentoEmpresa(chk_array($this->form_data, 'idEmpresa'), chk_array($this->form_data, 'tipo'), chk_array($this->form_data, 'titulo'), chk_array($this->form_data, 'documento'), chk_array($this->form_data, 'detalhes'));
	...
	public function getDocumentosEmpresa($idEmpresa = null){
		return $this->ClasseDocumento->getDocumentosEmpresa($idEmpresa);
	}
	...
	public function removerDocumento($parametros = array()){
		...
		if(chk_array($parametros, 3) == 'remover'){
			$this->form_msg = $this->controller->Messages->questionYesNo('Tem certeza que deseja apagar este endereço?', 'Sim', 'Não', $_SERVER['REQUEST_URI'] . '/confirma',  HOME_URI . '/empresas/index/editar/'. chk_array($parametros, 1) . '/documentos/');
		...
		if($id > 0){
			...
			$this->form_msg .= '<meta http-equiv="refresh" content="0; url=' . HOME_URI . '/empresas/index/editar/'. chk_array($parametros, 1) . '/documentos/">';
			$this->form_msg .= '<script type="text/javascript">window.location.href = "' . HOME_URI . '/empresas/index/editar/'. chk_array($parametros, 1) . '/documentos/";</script>';
			return;
		}
	}
}
