...
class EnderecosModel extends MainModel {
	...
	public function validarFormEndereco(){ 
		...
		$validaEndereco = $this->ClasseEndereco->validarEndereco($this->idPessoa, 0, $this->titulo, $this->cep, $this->logradouro, $this->numero, $this->complemento, $this->zona, $this->bairro, $this->cidade, $this->estado, $this->latitude, $this->longitude);
	...
	private function editarEndereco(){
		...
		if(!$editaEndereco){
			...
			$this->form_msg .= '<meta http-equiv="refresh" content="2; url=' . HOME_URI . '/pessoas/index/editar/' . chk_array($this->parametros, 1) .'/enderecos/">';
		}
	...
	private function adicionarEndereco(){
		$insereEndereco = $this->ClasseEndereco->adicionarEndereco(chk_array($this->form_data, 'idPessoa'), chk_array($this->form_data, 'titulo'), chk_array($this->form_data, 'cep'), chk_array($this->form_data, 'logradouro'), chk_array($this->form_data, 'numero'), chk_array($this->form_data, 'complemento'), chk_array($this->form_data, 'zona'), chk_array($this->form_data, 'bairro'), chk_array($this->form_data, 'cidade'), chk_array($this->form_data, 'estado'), chk_array($this->form_data, 'latitude'), chk_array($this->form_data, 'longitude'));
	...
	public function getEnderecos($idPessoa = null){
		return $this->ClasseEndereco->getEnderecos($idPessoa);
	}
	...
	public function removerEndereco($parametros = array()){
		...
		if(chk_array($parametros, 3) == 'remover'){
			$this->form_msg = $this->controller->Messages->questionYesNo('Tem certeza que deseja apagar este endereço?', 'Sim', 'Não', $_SERVER['REQUEST_URI'] . '/confirma',  HOME_URI . '/pessoas/index/editar/'. chk_array($parametros, 1) . '/enderecos/');
		...
		if($id > 0){
			...
			$this->form_msg .= '<meta http-equiv="refresh" content="0; url=' . HOME_URI . '/pessoas/index/editar/'. chk_array($parametros, 1) . '/enderecos/">';
			$this->form_msg .= '<script type="text/javascript">window.location.href = "' . HOME_URI . '/pessoas/index/editar/'. chk_array($parametros, 1) . '/enderecos/";</script>';
			return;
		}
	}
}
